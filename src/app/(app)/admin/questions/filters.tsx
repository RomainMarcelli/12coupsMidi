    ...
    <div className="flex flex-col gap-3 rounded-xl border border-border bg-card/50 p-4 sm:flex-row sm:flex-wrap sm:items-end">
      ...
      {hasAny && (
        <button
          ...
          className="flex items-center gap-1 self-end rounded-md border border-border px-3 py-2 text-sm text-navy/80 transition-colors hover:border-buzz hover:text-buzz"
        >
      ...
      {isPending && (
        <span className="self-end text-xs text-navy/50">Mise à jour…</span>
      )}
    </div>
  );
}
  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      <select
        ...
        className="h-9 rounded-md border border-border bg-card px-3 text-sm text-navy focus:border-gold focus:outline-none"
      >
        {options.map((o) => (
          <option key={o.value} value={o.value} className="bg-cream-deep">
            {o.label}
          </option>
        ))}
      </select>
