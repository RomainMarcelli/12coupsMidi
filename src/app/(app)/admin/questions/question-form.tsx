    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Catégorie">
          ...
          >
            ...
            {categories.map((c) => (
              <option key={c.slug} value={c.slug} className="bg-muted">
                {c.nom}
              </option>
            ))}
          </select>
        ...
        <Field label="Sous-catégorie">
          <select
            ...
            className="h-10 w-full rounded-md border border-border bg-card px-3 text-sm text-foreground focus:border-gold focus:outline-none disabled:opacity-50"
          >
            <option value="" className="bg-muted">
              — Aucune —
            </option>
            {availableSubcats.map((s) => (
              <option key={s.slug} value={s.slug} className="bg-muted">
                {s.nom}
              </option>
            ))}
          </select>
      ...
      <Field label="Énoncé">
        <textarea
          ...
          required
          minLength={3}
          className="w-full rounded-md border border-border bg-card p-3 text-sm text-foreground focus:border-gold focus:outline-none"
        />
      </Field>
                ...
                <button
                  ...
                  className={
                    r.correct
                      ...
                      : "flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border text-foreground/40 hover:border-life-green"
                  }
                >
                  {r.correct && (
      ...
      {needsIndices && (
        ...
        >
          <textarea
            ...
            rows={6}
            className="w-full rounded-md border border-border bg-card p-3 font-mono text-sm text-foreground focus:border-gold focus:outline-none"
          />
        </Field>
      )}
